...
{
    volatile uint16_t LCD_REGREAD = 0;
    Clr_Cs;  /* Cs low */ 
    Set_Rs;
    /* selected LCD register */ 
    Lcd_Write_Index(LCD_Reg);
    /* Write register data */
    LCD_REGREAD = LCD_Read_Data(); 
    Set_Cs;  /* Cs high */
    Clr_Rs;
    /* return read data */
    return LCD_REGREAD;
...
//Инициализация
void LCD_ini(void){
	HAL_GPIO_WritePin(GPIOD,GPIO_PIN_6, GPIO_PIN_RESET);//LCD-CS
	HAL_Delay(100);
...
LCD_write_reg(0x00EF,  0x1231);      //  Set  internal  timing  
        ...
        LCD_write_reg(0x0003,  0x0030);         //  set  GRAM  write  direction  and  RGB=1.    
